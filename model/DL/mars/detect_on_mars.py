...
def preprocess_test_data(df):
    ...
    df['time_abs'] = pd.to_datetime(df['time(%Y-%m-%dT%H:%M:%S.%f)'], format='%Y-%m-%dT%H:%M:%S.%f')
    return df

# Function to normalize the velocity data
def normalize_velocity(df):
    scaler = MinMaxScaler()
    df['normalized_velocity'] = scaler.fit_transform(df[['velocity(c/s)']])
    return df
...
def butterworth_filter(df, lowcut=0.1, highcut=3.0, fs=6.625, order=5):
    ...
    df['filtered_velocity'] = filtfilt(b, a, df['velocity(c/s)'])
    return df
...
def detect_seismic_events_sta_lta(df, sta_len=120, lta_len=600, df_rate=6.625, threshold=2.5, min_distance=5000):
    ...
    for peak in filtered_peaks:
        if peak :  # Ensure index is within bounds
            ...
            if df['velocity(c/s)'].iloc[peak] < some_min_amplitude: 
                 
          
                event = {
                    'time_abs': df['time_abs'].iloc[peak],
                    'velocity': df['velocity(c/s)'].iloc[peak],
...
def test_seismic_detection_sta_lta(test_data_dir, sta_len=120, lta_len=600, df_rate=6.625, threshold=2.5, min_distance=5000):
    ...
    # Loop through each CSV file in the test data directory
...
test_data_dir = 'C:/Users/nm_ma/Desktop/space_apps_2024_seismic_detection/data/mars/test/data'
